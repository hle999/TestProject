...
public class RoundRectDrawableWithShadow extends Drawable {
    ...
    final float mInsetShadow=0; // extra shadow to avoid gaps between card and shadow
...
//        mInsetShadow = resources.getDimension(android.support.v7.cardview.R.dimen.cardview_compat_inset_shadow);
    ...
    public void draw(Canvas canvas) {
        ...
        canvas.translate(mRawShadowSize / 2, mRawShadowSize / 2);
        fillColor(canvas, mCardBounds, mCornerRadius, mRawShadowSize, mPaint);
        canvas.restoreToCount(saveId);
    }

    private void fillColor(Canvas canvas, RectF bounds, float cornerRadius, float rawShadowSize,
                           Paint paint) {
        final float twoRadius = cornerRadius * 2;
        final float innerWidth = bounds.width() - twoRadius - rawShadowSize;
        final float innerHeight = bounds.height() - twoRadius - rawShadowSize;
        ...
        canvas.drawRect(bounds.left + cornerRadius, bounds.top,
                bounds.right - cornerRadius - rawShadowSize, bounds.top + cornerRadius,
                paint);
        canvas.drawRect(bounds.left + cornerRadius,
                bounds.bottom - cornerRadius - rawShadowSize, bounds.right - cornerRadius - rawShadowSize,
                bounds.bottom - rawShadowSize, paint);

        //center
        canvas.drawRect(bounds.left, bounds.top + cornerRadius,
                bounds.right - rawShadowSize, bounds.bottom - cornerRadius - rawShadowSize, paint);
    }
